...
def process_sample(args, sample, kg):
            ...
            for i in range(len(ctx[1])):
                if (ent in kg) and (str(i) in kg[ent]) and (len(kg[ent][str(i)]) > 0):
                    ...
                    for triplet in target_kg:
                        h, r, t = triplet
                        if (
                            ngram_overlap(h, ent) >= 0.90
                            or ngram_overlap(ent, h) >= 0.90
                        ):
                            h = ent
                        if (
                            ngram_overlap(t, ent) >= 0.90
                            or ngram_overlap(ent, t) >= 0.90
                        ):
                            t = ent
                        triplet = (h, r, t)
    ...
    try:
        ...
        if args.dataset == "hotpotqa":
            sps = [
                [
                    node.node.id_.split("##")[0],
                    int(node.node.id_.split("##")[1]),
                ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Whether to use triplet representation",
    )
    parser.add_argument(
        "--result_path",
        type=str,

    ...
    def _postprocess_nodes(
        ...
        for i in range(0, len(sorted_nodes)):
            ...
            if prev_ent == temp_ent:
                sorted_nodes[i].node.text = sorted_nodes[i].node.text[
                    len(temp_ent + ": ") :
                ]
...
class KGRetrievePostProcessor(BaseNodePostprocessor):
    ...
    def _postprocess_nodes(
        ...
        sorted_ents = sorted(
            ent_count.keys(), key=lambda x: (ent_score[x] / ent_count[x]), reverse=True
        )
        ...
        for ent in related_ents - retrieved_ents:
            if (
                (ent not in self.chunks_index)
                or (len(self.chunks_index[ent]) == 0)
                or (ent not in self.doc2kg)
            ):
                continue
            for idx_seq_str in self.chunks_index[ent]:
                ...
                if ent in ent_score:
                    textid2score[ctx_id] += (ent_score[ent] + avg_score) / (
                        ent_count[ent] + 1
                    )
    ...
    def _postprocess_nodes(
        ...
        for node in nodes:
            if self.dataset == "hotpotqa":
                ...
                idx_seq_str = f"{idx_str}##{seq_str}"
            if (
                (ent not in self.doc2kg)
                or (idx_seq_str not in self.doc2kg[ent])
                or (len(self.doc2kg[ent][idx_seq_str]) == 0)
            ):
                continue
            for triplet in self.doc2kg[ent][idx_seq_str]:
                h, r, t = triplet
        ...
        for i in range(len(mentioned_ents_list)):
            for j in range(i + 1, len(mentioned_ents_list)):
                if (g.has_edge(mentioned_ents_list[i], mentioned_ents_list[j])) or (
                    ...
                    continue
                g.add_edge(
                    mentioned_ents_list[i],
                    mentioned_ents_list[j],
                    rel="cooccurrence",
                    source="query",
                    weight=0.0,
                )
        ...
        for i in range(len(sorted_wccs)):
            ...
            if len(wcc) > 1:
                ...
                sorted_edges = sorted(
                    g.subgraph(wcc).edges(data=True),
        ...
        for cand_ids_list in cand_ids_lists:
            ...
            for cand_id in cand_ids_list:
                ...
                if (
                    (self.use_tpt)
                    and (cand_ent in self.doc2kg)
                    and (idx_seq_str in self.doc2kg[cand_ent])
                    and (len(self.doc2kg[cand_ent][idx_seq_str]) > 0)
                ):
                    tpt_str += ", ".join(
                        [
                            f"{h} has/is {r} {t}"
                            for h, r, t in self.doc2kg[cand_ent][idx_seq_str][
        ...
        if len(cand_strs) == 0:
            ...
            wanted_nodes = [
                nodes[sorted_seqs[i]] for i in range(min(self.topk, len(sorted_seqs)))
            ]
            return wanted_nodes
